...
import {AccsFormEdit} from "./AccsFormEdit.js";

class Accs extends FormView {
    ...
    Start(id) {
        this.ModuleId = id;
        LoadForm("#" + this.ModuleId, this.GetUrl("/Accs/AccsFormList?prefix=" + this.prefix), this.InitFunc.bind(this));
    }

    InitFunc() {
        this.InitComponents(this.ModuleId, this.prefix);
        AddKeyboardNavigationForGrid(this.dgAccs);
        this.dgAccs.datagrid({
            onLoadError: (data) => {
                this.ShowErrorResponse(data.responseJSON);
            },
            rowStyler: this.dgAccs_rowStyler.bind(this),
            onLoadSuccess: this.dgAccs_onLoadSuccess.bind(this),
        ...
        if (this.prefix == "modal_") {
            ...
            this.wAccs = $("#" + this.ModuleId);
            this.InitCloseEvents(this.wAccs, false);
            this.btnCancel.linkbutton({
                onClick: function () {
                    this.wAccs.window("close")
                }.bind(this)
            });
    ...
    LoadRights() {
        $.ajax({
            method: "get",
            url: this.GetUrl('/CoreUtils/GetActRights?TaskCode=Accs.dll&ActCode=AccsChange'),
     ...
     */
    btnDelete_onClick(){
        if(this.dgAccs.datagrid("getRows").length == 0) {
            this.ShowWarning("Нет записей для удаления");
            return false;
        }
        let selData = this.dgAccs.datagrid("getSelected");
        if(selData==null) {
        ...
        $.messager.confirm(header, "Вы действительно хотите " + action + " выделенный счет \"" + selData.name + "\" с Id = " + selData.id + "?",
            function (result) {
                if (result) {
                    this.sLoc.StateLockRecord("accs", selData.id, this.btnContinueDelete_onClick.bind(this));
                }
            }.bind(this));
    }
    ...
    /**
     * Обновление спиcка счетов
     */
    btnUpdate_onClick() {
        let row = this.dgAccs.datagrid("getSelected");
        if (row != null) {
            this.AccsIndex = this.dgAccs.datagrid("getRowIndex", row);
            if (this.AccsIndex < 0) {
                this.AccsIndex = 0;
            }
        }
        ...
        this.dgAccs.datagrid({url: this.GetUrl('/Accs/list?del=' + del)});
    }

    /**
     * Обработка окончания загрузки списка счетов
     * @param data - информация о загруженных данных
     */
    dgAccs_onLoadSuccess(data) {
        if (data.total > 0) {
            if (this.AccsId != -1) {
                this.dgAccs.datagrid("selectRecord", this.AccsId);
            } else {
                if (this.AccsIndex >= 0 && this.AccsIndex < data.total) {
                    this.dgAccs.datagrid("selectRow", this.AccsIndex);
                } else if (data.total > 0) {
                    this.dgAccs.datagrid("selectRow", data.total - 1);
                }
            }
    ...
    btnChange_onClick() {
        ...
        if (this.editMode) {
            this.sLoc.LockRecord("accs", selData.id, this.btnContinueChange_onClick.bind(this));
        } else {
    ...
    btnContinueChange_onClick(options){
        ...
        let form = new AccsFormEdit();
        form.SetResultFunc((RecId)=>{  this.AccsId = RecId; this.btnUpdate_onClick();});
        form.SetCloseWindowFunction((options)=>{
            if(options!=null){
                if(options.lockState){
                    this.sLoc.FreeLockRecord("accs", options.id);
                }
            }
        });
        form.Show(options);
...
export function StartModalModul(StartParams, ResultFunc) {
    let id = "wAccs_Module_Accs_AccsFormList";
    CreateModalWindow(id, "Справочник счетов");
    let form = new Accs("modal_", StartParams);
