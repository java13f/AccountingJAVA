    ...
    initDataGrid(dg, btnDel, options) {
        ...
        dg.datagrid({
            ...
            onLoadSuccess: (data) => {
                ...
                if (dg.frst == null) {
                    dg.frst = 0;
                    dg.datagrid({data: dg.datagrid('getData')});
                }
                if (options.onLoadSuccess != null)
                    options.onLoadSuccess(data);
            },
            onSelect: (record) => {
                dg.index = dg.datagrid("getRowIndex", dg.datagrid("getSelected"));
                if(dg.datagrid("getData").rows[dg.datagrid("getRowIndex", dg.datagrid("getSelected"))].id!=null)
                    dg.id=dg.datagrid("getData").rows[dg.datagrid("getRowIndex", dg.datagrid("getSelected"))].id;


                if (btnDel != null) {
                    btnDel.linkbutton({iconCls: "icon-remove", text: "Удалить"});
                    if (this.dg.datagrid("getRows").length != 0) {
                        let selData = this.dg.datagrid("getSelected");
                        if (selData != null) {
                            if (selData.del == 1) {
                                this.btnDeleteMsg.linkbutton({iconCls: "icon-undo", text: "Вернуть"});
                            }
                        }
                    }
                }
